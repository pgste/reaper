...
- **Phase F (S shipped + M designed):** Partial evaluation for the compiled DSL path — added after Phase C landed, as the follow-through on the intra-policy half that the pruning index does not capture. **F.1 (shipped):** tier-1 deploy-time constant folding (`compiler::fold_condition` — double-negation, And/Or constant propagation and splicing, gated on the variable-binding soundness guard; pinned by the compiled-vs-AST equivalence differential). **F.2-F.5 (designed, not built):** tier-2 data-dependent specialization — epoch-stamped specialized-rule overlay with fail-safe fallback to the general rules, behind `REAPER_USE_SPECIALIZED_POLICIES`, with a dry-run fitness decision point before the overlay is built. Full design, soundness gates, and the disposition of the legacy `partial_evaluation.rs`/`CompiledPolicyEvaluator` artifacts: `docs/development/PARTIAL_EVALUATION.md`.
