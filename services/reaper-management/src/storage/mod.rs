...
pub async fn create_storage(config: &StorageConfig) -> Result<Arc<dyn BundleStorage>, StorageError> {
    match config.storage_type.as_str() {
        ...
        #[cfg(feature = "storage-s3")]
        "s3" => {
            let bucket = config.s3.bucket.as_ref()
                .ok_or_else(|| StorageError::Config("S3 bucket not configured".to_string()))?;
            let region = config.s3.region.as_ref()
        ...
        #[cfg(feature = "storage-mongodb")]
        "mongodb" => {
            let uri = config.mongodb.uri.as_ref()
                .ok_or_else(|| StorageError::Config("MongoDB URI not configured".to_string()))?;
            let database = config.mongodb.database.as_ref()
                .ok_or_else(|| StorageError::Config("MongoDB database not configured".to_string()))?;
        ...
        #[cfg(feature = "storage-dynamodb")]
        "dynamodb" => {
            let table = config.dynamodb.table.as_ref()
                .ok_or_else(|| StorageError::Config("DynamoDB table not configured".to_string()))?;
            let region = config.dynamodb.region.as_ref()
                .ok_or_else(|| StorageError::Config("DynamoDB region not configured".to_string()))?;
        ...
        other => Err(StorageError::Config(format!(
            ...
            other,
            if cfg!(feature = "storage-s3") { ", s3" } else { "" },
            if cfg!(feature = "storage-mongodb") { ", mongodb" } else { "" },
            if cfg!(feature = "storage-dynamodb") { ", dynamodb" } else { "" },
        ))),
    }
}
