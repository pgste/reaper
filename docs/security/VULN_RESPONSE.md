...
| RUSTSEC-2024-0436 | `paste` 1.0 | **Unmaintained, not a vulnerability** (no CVE) — a **build-time** proc-macro helper (emits no runtime code, not in any shipped binary) pulled only by `utoipa-axum` 0.2, the OpenAPI contract generator (Plan 07). Used in its `routes!` macro; every published version depends on it, so there is no drop-in fix short of replacing utoipa-axum. `paste` is feature-complete and stable (archived because it was "done"). | maintainers | utoipa-axum moves off paste |
